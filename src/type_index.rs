...
mod tests {
    ...
    fn test_variable_index() {
        let mut index = TypeIndex::<String>::new();

        let var_a = Type::Variable(Variable::new("A".to_string()));
        let var_b = Type::Variable(Variable::new("B".to_string()));

        index.insert(&var_a, "item1".to_string());
        index.insert(&var_a, "item2".to_string());
        index.insert(&var_b, "item3".to_string());

        let results = index.find_variable("A");
        assert_eq!(results.len(), 2);

        let results = index.find_variable("B");
        assert_eq!(results.len(), 1);

        let results = index.find_variable("C");
        assert_eq!(results.len(), 0);
    }

    #[test]
    fn test_application_index() {
        Python::initialize();
        Python::attach(|py| {
            let mut index = TypeIndex::<String>::new();

            let var_a = pyo3::Py::new(py, Variable::new("A".to_string())).unwrap();
            let var_b = pyo3::Py::new(py, Variable::new("B".to_string())).unwrap();
            let app = Application::new(var_a.into(), var_b.into()).unwrap();
            let app_type = Type::Application(app);

            index.insert(&app_type, "item1".to_string());

            let var_a_type = Type::Variable(Variable::new("A".to_string()));
            let var_b_type = Type::Variable(Variable::new("B".to_string()));

            let results = index.find_application(&var_a_type, &var_b_type);
            assert_eq!(results.len(), 1);
        });
    }
    ...
    fn test_wildcard_match() {
        ...
        index.insert(&var_a, "item1".to_string());
        index.insert(&var_b, "item2".to_string());

        let all = index.find_all();
        assert_eq!(all.len(), 2);
    }

    #[test]
    fn test_partial_application_match() {
        Python::initialize();
        Python::attach(|py| {
            let mut index = TypeIndex::<String>::new();

            let var_a_py = pyo3::Py::new(py, Variable::new("A".to_string())).unwrap();
            let var_b_py = pyo3::Py::new(py, Variable::new("B".to_string())).unwrap();
            let var_c_py = pyo3::Py::new(py, Variable::new("C".to_string())).unwrap();

            let app1 = Application::new(var_a_py.clone_ref(py).into(), var_b_py.into()).unwrap();
            let app2 = Application::new(var_a_py.into(), var_c_py.into()).unwrap();

            index.insert(&Type::Application(app1), "A->B".to_string());
            index.insert(&Type::Application(app2), "A->C".to_string());

            let var_a_type = Type::Variable(Variable::new("A".to_string()));
            let results = index.find_application_by_left(&var_a_type);
            assert_eq!(results.len(), 2);
        });
    }
}
